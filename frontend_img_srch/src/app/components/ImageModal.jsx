...
import { X, Copy, FolderOpen, Maximize2 } from 'lucide-react';
...
const ImageModal = ({
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="max-w-5xl w-full max-h-[90vh] p-4 relative" onClick={(e) => e.stopPropagation()}>
        <button
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-4">
            <div className="flex items-center justify-between">
              <p className="text-xl font-semibold">
                Similarity Score:
                <span className={`ml-2 ${getScoreColor(image.score)}`}>{(image.score * 100).toFixed(2)}%</span>
              </p>
              <Button
                variant="outline"
                size="sm"
            ...
            <p className="text-sm text-gray-400 mt-2 text-center italic">Double-click image to open in default viewer</p>
          </div>
        </div>
      </div>
    </div>
